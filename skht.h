	...
	*/
	void __insert(Kmer_queue_r *q)
	{
		...
		if (!hashtable[pidx].occupied)
		{
			...
			this->num_memcpys++;
#endif
			memcpy(&hashtable[pidx].kmer_data, q->kmer_data_ptr,
				   KMER_DATA_LENGTH);
...
public:
	...
	SimpleKmerHashTable(uint64_t c)
	{
		...
		this->hashtable = (Kmer_r *)aligned_alloc(PAGE_SIZE,
												  capacity * sizeof(Kmer_r));
...
{
	return strm << std::string(k.kmer_data, 
		KMER_DATA_LENGTH) << " : " << k.kmer_count;
}
