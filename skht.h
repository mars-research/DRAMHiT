...
typedef struct {
	...
	uint32_t init_bucket_idx; // 4 bytes
...
typedef struct {
	__Kmer_r kmer; //57 bytes
	uint32_t prefetch_bucket_idx; // 4 bytes
	char padding[3];
...
private:
	...
	// TODO inline
	uint32_t __distance_to_bucket(uint32_t org_bkt_idx, uint32_t prb_bkt_idx)
	{
		return (prb_bkt_idx - org_bkt_idx) & (this->capacity -1); // modulo
	}
	...
	*/
	...
	bool __swap(__Kmer_r* table_kmer, __Kmer_r* queue_kmer, uint32_t idx){
		...
		*table_kmer = *queue_kmer;
		...
		queue[this->queue_idx].prefetch_bucket_idx = next_bucket_idx_to_prefetch; 
	...
	{
		uint32_t i = cache_record->prefetch_bucket_idx;
		...
		} else if ( __distance_to_bucket(hashtable[i].kmer.init_bucket_idx, i) <
				__distance_to_bucket(cache_record->kmer.init_bucket_idx, i))
		{
				__swap(&hashtable[i].kmer, &cache_record->kmer, i);

		} else {
			uint32_t next_bucket_idx_to_prefetch = (i + 1) & (this->capacity - 1); 
			__builtin_prefetch(&hashtable[next_bucket_idx_to_prefetch], 1, 3);
			cache_record->prefetch_bucket_idx = next_bucket_idx_to_prefetch;
...
public: 
	...
	bool insert(const base_4bit_t* kmer_data) 
	{
		...
		queue[this->queue_idx].kmer.init_bucket_idx = __kmer_idx;
		queue[this->queue_idx].prefetch_bucket_idx = __kmer_idx;
		this->queue_idx++;
