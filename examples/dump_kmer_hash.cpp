...
ABSL_FLAG(std::string, output_file, "hashes.data", "Output file path");
ABSL_FLAG(double, sample_rate, 1.0,
          "Fraction of output sampled. 1.0 being sampling all output.");

void dump_hashes(const std::string_view input_file,
                 const std::string_view output_file, double sample_rate) {
  std::cerr << "Reading from " << input_file << " writing to " << output_file
            << " with sampling rate " << sample_rate << " with K=" << KMER_LEN
            << std::endl;
