...
#ifndef HASHTABLES_CAS23_KHT_HPP
...
class CAS23HashTable : public BaseHashTable {
 public:
...
KV *CAS23HashTable<KV, KVQ>::hashtable = nullptr;

template <class KV, class KVQ>
uint64_t CAS23HashTable<KV, KVQ>::empty_slot_ = 0;

template <class KV, class KVQ>
bool CAS23HashTable<KV, KVQ>::empty_slot_exists_ = false;

template <class KV, class KVQ>
std::mutex CAS23HashTable<KV, KVQ>::ht_init_mutex;

template <class KV, class KVQ>
uint32_t CAS23HashTable<KV, KVQ>::ref_cnt = 0;
}  // namespace kmercounter
