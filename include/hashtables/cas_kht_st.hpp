          ...
          do {
...

            {
              ...
              if (key_cmp > 0) {
                ...
                if constexpr (std::is_same_v<KV, Aggr_KV>) {
                  // Single-threaded table, so a plain increment is enough.
                  bucket[(offset + 1)] += 1;
                } else {
            ...
            try_insert:
              ...
              */
              ...
              // bucket-full path above went to N+1, skipping N+1 entirely and
              // letting an insert miss a key already sitting there.
            retry_add_to_queue:
