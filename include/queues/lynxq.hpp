...
enum section_state {
  ...
  PUSH_WRITES = 1 << 1,
  /* Section is ready for the pop() to step in. Push() has written values into
     it. */
  PUSH_READY = 1 << 2,
...
#define DEFINE_PUSH_OVERLOADED_NEW(TYPE, MOV)                           \
  inline void push(TYPE data) {                                         \
    ...
    asm(MOV " %0, (%1, %2)"                                             \
        : /* no outputs */                                              \
        : "r"(data), "r"(free_push_reg),                                \
          "r"((TYPE *)push_index_tmp) /* inputs */                      \
        : "1");                                                         \
...
struct alignas(4096) lynxQ {
  ...
  void *operator new(std::size_t size,
    ...
    auto ptr = aligned_alloc(static_cast<std::size_t>(queue_align),
                             size + qsize + (1 << 12));
    ...
    //       << ", align: " << static_cast<std::size_t>(align) << ", ptr: " <<
    //       ptr;
    return ptr;
  }
...
const char *lynxQ::config_red_zone(on_or_off_t cond, void *addr) {
  ...
  if (!addr) {
    ...
    if (strings != NULL) {
      ...
      for (auto i = 0u; i < size; i++) printf("%s\n", strings[i]);
    }
  }
...
void lynxQ::pop_done(void) { return; }
...
static inline void set_reg_value(ucontext_t *uc, int regid, uint64_t value,
                                 csh handle) {
  switch (regid) {
    case X86_REG_RAX:
...
static inline void collect_regs(Regs *regs, cs_x86_op *mem_op, csh handle,
  ...
  assert(regs->reg_base != INT_MIN && regs->mem_disp != UINT_MAX &&
         "not initialized");
}
...
inline char *lynxQ::get_new_redzone_left(char *curr_redzone) {
  ...
  if (new_redzone < QUEUE) new_redzone = QUEUE + queue_size - REDZONE_SIZE;
  return new_redzone;
}
...
lynxQ::lynxQ(size_t qsize, struct prod_queue *pq, struct cons_queue *cq) {
  ...
  /* We need the addresses to be aligned with a larger alignment than
   queue_size. This is to allow for the circular address optimization: Example:
   Assuming a queue of size 4, QUEUE_ALIGN is 8. Therefore the addresses always
   have a '0' just before the index part to allow for overflow without modifying
  ...
  if (!(queue_size >= 4 * REDZONE_SIZE)) {
    fatal_error(
        "Queue (=0x%lx) too small compared to redzone (=0x%lx)\n"
  ...
  redzone2 = (char *)config_red_zone(
      ON, QUEUE + 2 * QUEUE_SECTION_SIZE + REDZONE_SIZE);
...
double lynxQ::get_time(void) { return queue_time; }
...
static lynxQ_t get_beginning_of_queue(char *ptr_in_queue) {
  ...
  for (qset_t::iterator it = all_queues_created.begin(),
                        ite = all_queues_created.end();
       it != ite; ++it) {
    lynxQ_t queue = *it;
...
static void redzone_do_push(const char *redzone_str, char *index,
...
static void redzone_do_pop(const char *redzone_str, char *index,
                           volatile section_state_t *sstate_prev,
                           volatile section_state_t *sstate_next,
                           char **redzone, char *QUEUE, lynxQ_t queue) {
...
std::tuple<csh, cs_x86_op *> decode_insn(const ucontext_t *context,
  ...
  if (ip != memo->ip) {
    ...
    int count = cs_disasm(handle, ip, 8 /*code_size*/, 0x0 /*addr*/,
                          1 /*num to disasm*/, &insn);
    if (count == 0) handle_error("cs_disasm failed.");
...
std::tuple<uint64_t, uint64_t> get_mem_base(const ucontext_t *context,
                                            Memoize_mem_op_t *memo,
                                            char *base_addr, csh handle,
                                            cs_x86_op *mem_op) {
  Regs regs;
  ...
  uint64_t sum = regs.mem_disp + new_base_value +
...
static void lynxQ_nomprotect_handler(int signal, siginfo_t *info, void *cxt) {
  ...
  if (index >= queue->QUEUE + queue->queue_size) {
    ...
    if (index_is_in_redzone(index, queue->redzone1)) {
      ...
      auto [new_base_value, reg_base_to_update] =
          get_mem_base(context, &memo, new_index, handle, mem_op);
      ...
      queue->dump();
      segfault(
          "*** PUSH: Index %p not in red-zone. This is a program bug ***\n",
          index);
    }
  } else if (memo.mem_type == LOAD_TYPE) {
    if (index_is_in_redzone(index, queue->redzone1)) {
    ...
    /* The index is not in the queue's red zone. This is a program bug. */
      ...
      queue->dump();
      segfault(
          "*** POP: Index %p not in red-zone. This is a program bug. ***\n",
          index);
    }
  } else {
    ...
    queue->dump();
    segfault(
        "*** Cannot detemine if %p is in push_expected_rz or pop_expected_rz. "
        "***\n",
        index);
  }
}
  ...
  if (index >= queue->QUEUE + queue->queue_size) {
    ...
    if (queue->allow_rotate) {
      ...
      if (memo.mem_type == STORE_TYPE) {
        ...
        //             old_push_reg, queue->pq_state->free_push_reg,
        //             queue->pq_state->push_index);
        queue->pq_state->push_index = queue->QUEUE;
        ...
        //       old_pop_reg, queue->cq_state->free_pop_reg,
        //       queue->cq_state->pop_index);
        queue->cq_state->pop_index = queue->QUEUE;
      } else {
        fprintf(stderr, "ERROR: Can't determine which thread it is\n");
      }
    ...
    if (index_is_in_redzone(index, queue->redzone1)) {
      redzone_do_push("REDZONE 1", index, &queue->qstate.sstate0,
                      &queue->qstate.sstate1, &queue->redzone1, queue->QUEUE,
                      queue);
    } else if (index_is_in_redzone(index, queue->redzone2)) {
      redzone_do_push("REDZONE 2", index, &queue->qstate.sstate1,
                      &queue->qstate.sstate0, &queue->redzone2, queue->QUEUE,
                      queue);
    } else {
      fprintf(stderr, "\n\n");
      queue->dump();
...
#endif
        redzone_do_pop("REDZONE 1", index, &queue->qstate.sstate0,
                       &queue->qstate.sstate1, &queue->redzone1, queue->QUEUE,
                       queue);
      } else if (index_is_in_redzone(index, queue->redzone2)) {
#if 0
...
#endif
        redzone_do_pop("REDZONE 2", index, &queue->qstate.sstate1,
                       &queue->qstate.sstate0, &queue->redzone2, queue->QUEUE,
                       queue);
      }
      /* The index is not in the queue's red zone. This is a program bug. */
 ...
 public:
  ...
  void init_prod_queues() {
    ...
    for (auto p = 0u; p < nprod; p++) {
      ...
      auto pqueues = (prod_queue_t *)calloc(1, ncons * sizeof(prod_queue_t));
  ...
  void init_cons_queues() {
    ...
    for (auto c = 0u; c < ncons; c++) {
      ...
      auto cqueues = (cons_queue_t *)calloc(1, nprod * sizeof(cons_queue_t));
