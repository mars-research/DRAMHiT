...
inline void print_stats(Shard *all_sh, Configuration &config) {
  ...
  for (; k < config.num_threads; k++) {
    //     printf(
    //         "Thread %2d: "
    //         "%" PRIu64 " cycles (%f ms) for %" PRIu64 " insertions (%" PRIu64
    //         " cycles/insert) | (%" PRIu64 " cycles/enqueue) "
    ...
    //         1000000.0, all_sh[k].stats->insertions.op_count,
    ...
    //             ? 0
    //             : (double)all_sh[k].stats->ht_fill /
    //             all_sh[k].stats->ht_capacity *
    //                   100
    // #ifdef CALC_STATS
    //         ,
...
#endif  // CALC_STATS
  }  // end config.num_threads loop

#ifdef CALC_STATS
  // printf("total reprobes %llu, spill factor ~(inserts+reprobes)/inserts:
  // %f\n",
  //   all_total_reprobes,
  ...
  //  printf("===============================================================\n");
  //  printf(
  //      "Average  : %" PRIu64 " cycles (%f ms) for %" PRIu64 " insertions (%"
  //      PRIu64 " cycles/insert) "
  //      "(fill = %u %%)\n",
  ...
  //  printf(
  //      "Average (find): %" PRIu64 " cycles (%f ms) for %" PRIu64 " finds (%"
  //      PRIu64 " cycles per " "find)\n", all_total_find_cycles /
  //      config.num_threads, (double)all_total_find_time_ns * one_cycle_ns /
  //      1000, kmer_big_pool_size_per_shard, all_total_find_cycles /
  //      config.num_threads /
  //          kmer_big_pool_size_per_shard);

  printf("===============================================================\n");
  printf("Total  : %" PRIu64 " cycles for %" PRIu64 " insertions\n",
  ...
  double find_mops = 0.0, insert_mops = 0.0, upsert_mops = 0.0;
  {
    ...
    if (total_finds > 0) {
      ...
      printf(
          "===============================================================\n");
      if (config.rw_queues) {
        printf("Average  : %" PRIu64 " cycles for %" PRIu64
               " combined R/W (%" PRIu64 " cycles/op)\n",
               total_find_cycles / config.n_prod, total_finds / config.n_prod,
               cycles_per_find);
      } else {
        // printf("Average  : %" PRIu64 " cycles for %" PRIu64 " finds\n",
        //       total_find_cycles / config.num_threads,
        //       total_finds / config.num_threads);
        // printf("===============================================================\n");

        printf("Total  : %" PRIu64 " cycles for %" PRIu64 " finds (%" PRIu64
               " cycles/find)\n",
               total_find_cycles, total_finds, cycles_per_find);
      }
      printf(
          "===============================================================\n");
    }
    ...
    if (total_upsert > 0)
      cycles_per_upsert = total_upsert_cycles / total_upsert;
    upsert_mops = ((double)CPUFREQ_MHZ / cycles_per_upsert) * num_threads;
    // for inserts, we only use n_cons
    if (config.mode == BQ_TESTS_YES_BQ) {
      num_threads = config.n_cons;
    }
    insert_mops =
        ((double)CPUFREQ_MHZ / cycles_per_insert) * num_threads;  // old one
    ...
    find_mops = ((double)CPUFREQ_MHZ / cycles_per_find) *
                (config.rw_queues ? config.n_prod : num_threads);
