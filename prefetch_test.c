...
But once store buffer is saturated, a load request for this store can't be issued
until it enters store buffer. It must wait until one entry in the 
store buffer to retire, which in turns waits for the LFB request to complete.

However, with prefetch, even when store buffer is saturated, the load request for 
a store can still be issued as long as lfb slot is freed. The load request is issued 
parallelly at this point. Note that store will still stall, waiting to enter. But 
when it enters, it is likely that the cacheline is already ready to go, thus 
leaving store buffer immediately. This is not true in the case of continuous stores.
*/
