...
void sync_complete(void) {
      ...
      if (cur_phase == ExecPhase::finds) {
        ...
        if (!zipfian_inserts) {
          ...
          if (zipfian_iter < config.insert_factor) {
            ...
            PLOGV.printf("insert duration %lu",
  ...
  void Application::shard_thread(int tid,
    ...
    sh->stats = (thread_stats *)std::aligned_alloc(CACHE_LINE_SIZE,
                                                   sizeof(thread_stats));
    ...
    switch (config.mode) {
      ...
      case BW:
      case HASHJOIN:
      case PARTITIONJOINV1:
      case PARTITIONJOINV2:
        kmer_ht = NULL;
        break;
      case BQ_TESTS_NO_BQ:
        kmer_ht = init_ht(config.ht_size, sh->shard_idx);
  ...
  int Application::spawn_shard_threads() {
    ...
    for (uint32_t assigned_cpu : this->np->get_assigned_cpu_list()) {
      ...
      if (sh->numa_node < 0) {
        PLOGE.printf("unknown node for shard %u cpu %u", sh->shard_idx,
                     sh->assigned_cpu);
        abort();
      } else {
        PLOGI.printf("shard %u assigned to %u at numa node %u", sh->shard_idx,
                     sh->assigned_cpu, sh->numa_node);
      }
      sh_idx++;
    }
    ...
    if (config.mode == FASTQ_WITH_INSERT) {
      ...
      if (config.ht_type == CASHTPP || config.ht_type == MULTI_HT ||
          ...
          config.ht_type == TBB_HT || config.ht_type == DLHT_HT || config.ht_type == FOLKLORE_HT) {
    ...
    for (uint32_t i = 1; i < config.num_threads; i++) {
      ...
      pthread_setaffinity_np(_thread.native_handle(), sizeof(cpu_set_t),
                             &cpuset);
  ...
  int Application::process(int argc, char *argv[]) {
    try {
      ...
      desc.add_options()("help", "produce help message")(
          ...
          "numa-split",
          po::value<uint32_t>(&config.numa_split)
              ->default_value(def.numa_split),
          ...
          "Path to relation S.")(
          "relation_r_size",
          po::value(&config.relation_r_size)
              ->default_value(def.relation_r_size),
          "Number of elements in relation R. Only used when the relations are "
          "generated.")(
          "relation_s_size",
          po::value(&config.relation_s_size)
              ->default_value(def.relation_s_size),
          "Number of elements in relation S. Only used when the relations are "
          "generated.")(
          "delimitor",
          po::value(&config.delimitor)->default_value(def.delimitor),
...
#ifdef WITH_PERFCPP
      EVENTCOUNTERS = MultithreadCounter(
          config.num_threads, config.perf_cnt_path, config.perf_def_path);
