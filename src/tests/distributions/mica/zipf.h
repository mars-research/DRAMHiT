...
double zeta(uint64_t last_n, double last_sum, uint64_t n,
                     double theta) {
  if (last_n > n) {
    last_n = 0;
    last_sum = 0.;
...
/*class Base_ZipfGen {
 ...
 protected:
  ...
  double theta_ = 0.0;  // skewness (input) in (0, 1); or, 0 = uniform, 1 = always zero
  double alpha_ = 0.0;     // only depends on theta
  double thres_ = 0.0;     // only depends on theta
  uint64_t last_n_ = 0;  // last n used to calculate the following
  double dbl_n_ = 0.0;
  double zetan_ = 0.0;
  double eta_ = 0.0;
...
{
  public:
	Uniform_ZipfGen(uint64_t n, double theta, uint64_t rand_seed):Base_ZipfGen(n, theta, rand_seed){}
	uint64_t next();
};
class Single_ZipfGen : public Base_ZipfGen
{
  public:
	Single_ZipfGen(uint64_t n, double theta, uint64_t rand_seed):Base_ZipfGen(n, theta, rand_seed){}
	uint64_t next();
};
class Theta_ZipfGen : public Base_ZipfGen
{
  public:
	Theta_ZipfGen(uint64_t n, double theta, uint64_t rand_seed):Base_ZipfGen(n, theta, rand_seed){}
	uint64_t next();
};
class Large_ZipfGen : public Base_ZipfGen
{
  public:
	Large_ZipfGen(uint64_t n, double theta, uint64_t rand_seed):Base_ZipfGen(n, theta, rand_seed){}
	uint64_t next();
};*/
