...
uint64_t SynthTest::synth_run(KmerHashTable *ktable) {
  ...
  //Depending on selected macro, give different data
		...
		*((uint64_t *)&kmers[k].data) = mine::next(0.33, -1.47, -0.272179, HT_TESTS_NUM_INSERTS, rand_);
		...
		*((uint64_t *)&kmers[k].data) = mem[i];
		//printf("%lu\n", mem[i]);
  #elif defined(HEADER)
		*((uint64_t *)&kmers[k].data) = next();//ZipfGen(HT_TESTS_NUM_INSERTS, theta_arg, HT_TESTS_NUM_INSERTS);//next();//1;//
		//printf("%lu\n", next());
	#else
...
void SynthTest::synth_run_exec(Shard *sh, KmerHashTable *kmer_ht) {
  ...
  for (auto i = 1; i < HT_TESTS_MAX_STRIDE; i++) {
    ...
    #elif defined(ZIPF)
      printf("theta_arg = %f\n", theta_arg);
      Base_ZipfGen* zg = ZipfGen(HT_TESTS_NUM_INSERTS, theta_arg, HT_TESTS_NUM_INSERTS);
    #elif defined(MINE)
      Rand* rand_ = new Rand();
    #elif defined(HEADER)
