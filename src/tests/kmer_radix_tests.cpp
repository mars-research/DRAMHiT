...
uint64_t partitioning(Shard* sh, const Configuration& config, RadixContext& context, 
  ...
  for (uint64_t kmer; reader->next(&kmer);) {
    ...
    if (slotMod == (KMERSPERCACHELINE - 1)) {
        ...
        // PLOGI.printf("partitions size: %llu, partition array: %llu, IDX: %u, idx: %u", context.partitions.size(), context.partitions[shard_idx].size(), shard_idx, idx);
        auto next_loc = partitionChunk->get_next();
        partitionChunk->advance();
      /* write out 64-Bytes with non-temporal store */
        store_nontemp_64B(next_loc,
                        (buffers + idx));
  ...
  auto swb_end = _rdtsc();
  PLOGI.printf("IDX: %u;SWB: %llu cycles; Timestamp: %llu; Partition_alloc: %llu", 
          shard_idx,
          swb_end - start_swb, 
...
void KmerTest::count_kmer_radix_custom(
        Shard* sh, 
        ...
        RadixContext& context) 
{
  ...
  //        PLOGI.printf("Iter: %u, alloc cycles: %llu, cycles per elem: %llu", i, end_ht, end_ht / num);
  ...
  PLOGI.printf("IDX: %u, time spent at the second barrier: %llu cycles", shard_idx, _rdtsc() - end_insertions_cycle);

  sh->stats->insertions.duration = _rdtsc() - start_shard;
  sh->stats->insertions.op_count = total_insertions;
  // PLOG_INFO.printf("IDX: %u, num_kmers: %u, fill: %u", shard_idx, num_kmers, ht->get_fill());
  for (uint32_t i = 0; i < hashmaps_per_thread; i++) {
    PLOG_INFO.printf("IDX: %u, cap: %u, fill: %u", shard_idx, maps[i]->get_capacity(), maps[i]->get_fill());
  }
  if (sh->shard_idx == 0) {
    end_ts = std::chrono::steady_clock::now();
    end_cycles = _rdtsc();
