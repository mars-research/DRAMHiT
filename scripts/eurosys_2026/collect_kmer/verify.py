...
def verify(truth, result, ignore_key0=True):
    all_keys = set(truth.keys()) | set(result.keys())

    # Aggr_KV (include/hashtables/kvtypes.hpp) uses key == 0 as its "empty"
    # sentinel, so the all-A kmer can never be stored or dumped. Counting it as
    # a miss makes every otherwise-perfect run look broken.
    if ignore_key0:
        all_keys.discard(0)
...
def main():
    ...
    parser.add_argument("--count-key0", action="store_true",
                        help="Also check the all-A kmer (encoded key 0). It is "
                             "Aggr_KV's empty sentinel and is never dumped, so "
                             "by default it is excluded from the comparison.")
    ...
    print("\nVerifying...")
    ignore_key0 = not args.count_key0
    mismatches, missing, extra = verify(truth, result, ignore_key0)
    ...
    if ignore_key0:
        print(f"(encoded key 0 excluded: Aggr_KV empty sentinel)")
    print(f"{'='*50}")
