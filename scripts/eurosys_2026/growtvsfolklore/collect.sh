...
if [ "$arch" = "amd" ]; then
    cmake -S $HOME_DIR -B $HOME_DIR/build -DCPUFREQ_MHZ=3250 -DDRAMHiT_VARIANT=2025_INLINE -DBUCKETIZATION=ON -DBRANCH=simd -DUNIFORM_PROBING=ON -DPREFETCH=DOUBLE -DCALC_STATS=OFF -DGROWT=ON
else
    cmake -S $HOME_DIR -B $HOME_DIR/build -DCPUFREQ_MHZ=2500 -DDRAMHiT_VARIANT=2025_INLINE -DBUCKETIZATION=ON -DBRANCH=simd -DUNIFORM_PROBING=ON -DPREFETCH=DOUBLE -DCALC_STATS=OFF -DGROWT=ON
fi
...
run_benchmark "folklore.txt"   "$FOLKLORE"
