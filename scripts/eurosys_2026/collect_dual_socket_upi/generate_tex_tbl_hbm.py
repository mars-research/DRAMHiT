...
def main():
    print("=== HBM LaTeX Table Generator ===")

    # 1. Get Snoop File
    snoop_file = input("Enter path for the SNOOP JSON file (e.g., hbm_snoop.json): ").strip()
    if not os.path.exists(snoop_file):
        print(f"Error: Could not find {snoop_file}")
        return

    # 2. Get Directory File
    dir_file = input("Enter path for the DIRECTORY JSON file (e.g., hbm_dir.json): ").strip()
    ...
    bw_s0_rd = lambda d, p, m, met: get_val(d, p, m, "bw", "S0", "unc_hbm_cas_count.rd", met)
    bw_s0_wr = lambda d, p, m, met: get_val(d, p, m, "bw", "S0", "unc_hbm_cas_count.wr", met)
    bw_s1_rd = lambda d, p, m, met: get_val(d, p, m, "bw", "S1", "unc_hbm_cas_count.rd", met)
    bw_s1_wr = lambda d, p, m, met: get_val(d, p, m, "bw", "S1", "unc_hbm_cas_count.wr", met)
...
\caption{D760 UPI + HBM traffic breakdown under Random """ + mode.capitalize() + r""" workload}
...
"""
    ...
    with open("hbm_table_output.tex", "w") as f:
        f.write(latex)
    print(f"\nLaTeX successfully saved to hbm_table_output.tex")
