...
def run_ht_dual(name: str, ht_type: int, hw_pref: int, results: dict):
    ...
    for exp in range(0, 4, 1):
        ...
        --batrh-len 16 --relation_r_size {rsize} """ cmd_base = " ".join(cmd_base.split())  # clean whitespace print(cmd_base)
...
if __name__ == "__main__":
    ...
    subprocess.run(
        ...
        "-DDRAMHiT_VARIANT=2025_INLINE -DCAS_NO_ABSTRACT=ON -DBUCKETIZATION=ON -DBRANCH=simd -DPREFETCH=DOUBLE -DUNIFORM_PROBING=ON "
        "-DGROWT=ON",
        shell=True,
        check=True,
    )
