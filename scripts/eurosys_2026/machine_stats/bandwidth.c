...
int main(int argc, char *argv[]) {
    ...
    double cpu_freq_ghz = 0.0; // NEW: store requested CPU frequency
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(argv[i], "-freq") == 0 && i + 1 < argc) cpu_freq_ghz = strtod(argv[++i], NULL); // NEW
    ...
    // UPDATED: Now requires cpu_freq_ghz via the -freq parameter
    if (raw_per_thread_size == 0 || pattern_str == NULL || cpu_freq_ghz <= 0.0) {
        fprintf(stderr, "Usage: %s -m <per_thread_size> -pattern \"n0a0t4...\" -freq <GHz> [-inst <load|avx512|t0|t1|t2|nta>] [-lookahead <lines>]\n", argv[0]);
        return -1;
    }
    ...
    // UPDATED: Now dynamically calculates the predicted peak bandwidth utilizing the user-supplied frequency
