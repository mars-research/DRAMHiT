	...
	for (size_t i = 0; i < big_pool_count; i++)
	{
		if (config.alphanum_kmers)
		{
			...
			std::string s = std::string(sh->kmer_small_pool[idx].data,
										KMER_DATA_LENGTH);
			memcpy(sh->kmer_big_pool[i].data,
				   sh->kmer_small_pool[idx].data,
				   KMER_DATA_LENGTH);
		}
		else
		{
			memcpy(sh->kmer_big_pool[i].data,
				   sh->kmer_small_pool[udist(gen)].data,
				   KMER_DATA_LENGTH);
		}
	}
}
...
void create_data(Shard *sh)
{
	...
	uint64_t KMER_BIG_POOL_COUNT = config.kmer_create_data_base *
	...
	std::string pool_filename_format =	config.kmer_files_dir + \
		std::string("%02u.bin");
